...
impl DashboardAggregator {
    ...
    pub fn get_metrics(&self) -> DashboardMetrics {
        ...
        for event in &filtered_events {
            *event_type_distribution.entry(event.event_type.to_string()).or_insert(0) += 1;
        }
        ...
        let top_mitre_techniques: Vec<_> = top_mitre.into_iter()
            .take(10)
            .map(|(tech, count)| MitreTechniqueCount { technique: tech, count })
            .collect();
    ...
    fn generate_time_series(&self, events: &[&WiaSecurityEvent]) -> Vec<TimeSeriesPoint> {
        ...
        let mut points: Vec<_> = hourly_counts.into_iter()
...
impl ComplianceTracker {
    ...
    pub fn new() -> Self {
        ...
        controls.insert(ComplianceFramework::Nist, vec![
            ComplianceControl {
                id: "ID.AM".to_string(),
        ...
        // CIS controls
        controls.insert(ComplianceFramework::Cis, vec![
            ComplianceControl {
                id: "CIS-1".to_string(),
    ...
    pub fn calculate_score_from_events(
        ...
        let mut covered_controls: std::collections::HashSet<String> = std::collections::HashSet::new();
...
mod tests {
    ...
    fn test_dashboard_metrics() {
        let config = DashboardConfig { time_window_hours: 24 };
