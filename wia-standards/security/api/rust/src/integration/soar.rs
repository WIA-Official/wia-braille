...
impl PhantomClient {
    ...
    pub async fn create_container(&self, event: &WiaSecurityEvent) -> SoarResult<String> {
        ...
        let _ = container;
        Ok(format!("phantom-container-{}", chrono::Utc::now().timestamp()))
    }
...
impl XsoarClient {
    ...
    /// Search incidents
    pub async fn search_incidents(&self, query: &str, size: usize) -> SoarResult<Vec<XsoarIncident>> {
        let _body = serde_json::json!({
            "filter": {
                "query": query,
            },
...
impl SwimlaneClient {
    ...
    pub async fn create_record(
        ...
        Ok(format!("swimlane-record-{}", chrono::Utc::now().timestamp()))
    }
